...
from keras.layers import merge, multiply, add, concatenate
...
def interp_block(
    ...
    if output_stride == 16:
        ...
        scale = 2
    kx = np.floor(feature_map_shape[0]/level)*scale
    ky = np.floor(feature_map_shape[1]/level)*scale
    kernel = (kx,ky)
    strides = (kx,ky)

    # print("feature map shape",feature_map_shape)
    # print("pooling x",x.shape,"level",level)
    # print("with kernel",kernel,"stride",strides,
    ...
    # print("output shape",global_feat.shape)
    return global_feat
